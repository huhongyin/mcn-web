                ...
                <el-table-column label="总流水(元)">
                    <template slot-scope="scope">
                        <template v-if="search.plat_id == 1">
                            {{ (scope.row.live_total_money == null) ? 0 : scope.row.live_total_money }}
                        </template>
...
export default {
    ...
    methods:{
        ...
        getData(){
            get(operateApi.groupCalList + '/money_list', this.search).then((res) => {
                this.totalPage = res.data.list.last_page
                this.list = res.data.list.data
                this.total = res.data.total
                this.countTxt = "共(" + res.data.total + ')元'
            })
        },
        getSummaries(param) {
